...
void setup()
{
  ...
  Button.attach(PIN_ENCODER_SWITCH);
}
...
void loop()
{
  ...
  if(ProcessPotRotation() || ProcessButton())
  {
    RequireDisplayUpdate();
  }
...
//---------------------------------------------------------
//---------------------------------------------------------
bool ProcessPotRotation()
{
  double PotVal = 0;
  for(uint8_t c = 0; c < SMOOTH_COUNT; c++){ // Gets SmoothCount Values of the Pot and sums them for later averaging
    PotVal += (analogRead(POT_PIN));   
  }
  PotVal = PotVal / SMOOTH_COUNT; // Averages the Summed values
  PotVal = floor(abs(PotVal / MAX_ANALOG_VAL) * 100); // Scales the Pot Values to 0-100
  int8_t PotDelta = floor(PotVal - PotVal_old); // Creates a Delta for any menuing
  
  if(abs(PotDelta) > COUNT_PER_TICK){
    PotVal_old = PotVal; // Saves Old Value
  }
  else{
    PotDelta = 0;
  }
  
  if(abs(PotDelta) == 0){ //Allows Button to work (Could change this to <1 or <2 incase it is too jumpy but ==0 works becuase flooring)
    return false;
  }
  ...
  if(mode == MODE_MASTER)
  {
    items[0].volume = PotVal; // To Exact Value of Pot
  ...
  else if(mode == MODE_APPLICATION)
  {
    if(stateApplication == STATE_APPLICATION_NAVIGATE)
      itemIndexApp = GetNextIndex(itemIndexApp, itemCount, (PotDelta/COUNT_PER_TICK), settings.continuousScroll);

    else if(stateApplication == STATE_APPLICATION_EDIT)
    {
      items[itemIndexApp].volume = PotVal; // To Exact Value of Pot
  ...
  else if(mode == MODE_GAME)
  {
    if(stateGame == STATE_GAME_SELECT_A)
      itemIndexGameA = GetNextIndex(itemIndexGameA, itemCount, (PotDelta/COUNT_PER_TICK), settings.continuousScroll);
    else if(stateGame == STATE_GAME_SELECT_B)
      itemIndexGameB = GetNextIndex(itemIndexGameB, itemCount, (PotDelta/COUNT_PER_TICK), settings.continuousScroll);

    else if(stateGame == STATE_GAME_EDIT)
    {
      items[itemIndexGameA].volume = PotVal; // To Exact Value of Pot
      items[itemIndexGameA].volume = constrain(items[itemIndexGameA].volume, 0, 100);

      items[itemIndexGameB].volume = 100 - PotVal; // To The opposite value of Pot
