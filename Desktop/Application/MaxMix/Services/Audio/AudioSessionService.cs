    ...
    internal class AudioSessionService : IAudioSessionService
    {
        ...
        {  
            ...
            if (!string.IsNullOrEmpty(fileName))
            {
                ...
                {
                    // We have a previously constructed group, so just add this session to that group and early out.
                    sessionGroup = group as AudioSessionGroup;
                    sessionGroup.AddSession(session_);
                    return;
                }

                // Need to create a new group for this session and register it
                sessionGroup = new AudioSessionGroup(groupID, session_.DisplayName);
                sessionGroup.AddSession(session_);
                session_ = sessionGroup;
            }
